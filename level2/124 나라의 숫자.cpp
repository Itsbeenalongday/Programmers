...
/* 진법 변환
10진수의 수를 2로 나누어 가며 그 나머지를 맨 오른쪽 수로 미루면서 왼쪽으로 진행한다.
3진법도 위와 마찬가지로 3으로 나누면서 그 나머지를 해당 자리수로 채울수 있다. 
그러나 124 나라에서 원하는건 원래 3진법에서 사용하는 0,1,2 대신 1,2,4 를 사용하는 것이다. 
그래서 '124'를 두고 3으로 나눈 나머지의 인덱스값을 각 자리수로 사용하는 것이다.
*/
